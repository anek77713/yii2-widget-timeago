 ...
 * @author Yakovlev Vladimir <yakovlev.vladimir@hotmail.com>
 * @since 2.0
 */
class TimeAgo extends Widget
{
    ...
    {
        if ($this->timestamp) {
            echo Html::tag($this->tag, Yii::$app->formatter->asDatetime($this->timestamp), ArrayHelper::merge($this->options, [
		'datetime' => Yii::$app->formatter->asDatetime($this->timestamp, "php:c"),
		'title' => Yii::$app->formatter->asDatetime($this->timestamp)
	]));
        }
    }
